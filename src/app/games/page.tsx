  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <header className="pt-8 pb-4 px-8 max-w-6xl mx-auto w-full flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <h1 className="text-4xl md:text-5xl font-black bg-gradient-to-br from-cyan-500 to-blue-600 bg-clip-text text-transparent flex items-center gap-3 drop-shadow-sm z-10">
          <div className="bg-blue-100 dark:bg-blue-900/30 p-2 rounded-2xl border-2 border-blue-200 dark:border-blue-800/50 shadow-inner">
            <Gamepad2 className="w-8 h-8 md:w-10 md:h-10 text-blue-500" />
          </div>
          Play Zone
        </h1>
          ...
          {games.map(game => (
            <Link key={game.id} href={`/games/${game.id}`}>
              <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all border-4 border-transparent hover:border-primary/50 relative overflow-hidden h-full flex flex-col cursor-pointer group">
                ...
                <div className={`rounded-2xl h-44 mb-6 flex items-center justify-center text-7xl group-hover:scale-105 transition-transform duration-300 shadow-inner bg-gradient-to-br ${game.color || "from-primary/20 to-primary/40"} border-4 border-white/20 relative overflow-hidden`}>
                  {/* Decorative blur */}
                ...
                <h3 className="text-2xl font-extrabold text-foreground mb-2">{game.title}</h3>
